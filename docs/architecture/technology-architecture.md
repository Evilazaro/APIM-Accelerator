...
flowchart TB
    ...
    Internet(["🌍 Internet / API Consumers"]):::external

    subgraph AzureCloud["☁️ Azure Cloud — Subscription"]
        subgraph ResourceGroup["📁 Resource Group — apim-accelerator-env-location-rg"]
            APIM("🔌 Azure API Management<br/>Premium SKU • 1 Unit"):::core
            APIMWorkspace("🗂️ APIM Workspace<br/>workspace1"):::core
            DevPortal("🌐 Developer Portal<br/>AAD Auth • MSAL 2.0"):::core
            APICenter("📋 Azure API Center<br/>Governance Platform"):::data

            subgraph MonitoringStack["📊 Monitoring Infrastructure"]
                LAW("📈 Log Analytics Workspace<br/>PerGB2018 SKU"):::success
                AppInsights("🔍 Application Insights<br/>Workspace-Based APM"):::success
                StorageAcct("💾 Storage Account<br/>Standard_LRS • StorageV2"):::neutral
            end
        end
    end
    ...
    %% Subgraph neutral styling — style directive only (MRM-S001 compliant)
...
- **Name Generation**: Auto-generated via `generateStorageAccountName(logAnalyticsWorkspaceName, uniqueString(resourceGroup().id))` — `src/shared/constants.bicep:155-160`; max 24 chars, lowercase, no hyphens
...
- Path: `/src/shared/monitoring/operational/` (partial match, has monitoring) (0.60) × 0.25 = 0.15
- Content: `Microsoft.Storage/storageAccounts`, `Standard_LRS`, `StorageV2`, diagnostic archival keywords (1.0) × 0.35 = 0.35
- Cross-reference: Referenced by `src/shared/monitoring/main.bicep`, `src/shared/monitoring/insights/main.bicep` (0.90) × 0.10 = 0.09
...
- Filename: `main.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/shared/networking/` (contains "networking") (0.70) × 0.25 = 0.175
- Content: `virtualNetwork`, `virtualNetworks`, network infrastructure keywords (0.80) × 0.35 = 0.28
- Cross-reference: Not imported by main orchestration chain; standalone module (0.20) × 0.10 = 0.02
...
- **Last Patched**: Azure-managed
- **EOL/EOS**: APIM API version `2025-03-01-preview` — current preview; `2024-10-01-preview` stabilized

**Confidence Score**: 0.92 (High)

- Filename: `apim.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/core/` (infrastructure Bicep) (0.70) × 0.25 = 0.175
- Content: `Microsoft.ApiManagement/service`, Premium SKU, VNet config, identity (1.0) × 0.35 = 0.35
- Cross-reference: Referenced by `src/core/main.bicep`, transitively by `infra/main.bicep` (0.95) × 0.10 = 0.095
...
- Filename: `main.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/inventory/` (0.50) × 0.25 = 0.125
- Content: `Microsoft.ApiCenter/services`, API source integration, identity, RBAC (0.90) × 0.35 = 0.315
- Cross-reference: Referenced by `infra/main.bicep` (0.90) × 0.10 = 0.09
...
- Filename: `main.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/infra/` matches `/infra/` pattern (1.0) × 0.25 = 0.25
- Content: `Microsoft.Resources/resourceGroups`, subscription scope, deployment keywords (1.0) × 0.35 = 0.35
- Cross-reference: Referenced by settings.yaml, all modules depend on it (0.80) × 0.10 = 0.08
...
- Filename: `developer-portal.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/core/` (0.70) × 0.25 = 0.175
- Content: CORS policy, security keywords, origin restriction (1.0) × 0.35 = 0.35
- Cross-reference: Referenced by `src/core/main.bicep` (0.85) × 0.10 = 0.085
...
- Filename: `developer-portal.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/core/` (0.70) × 0.25 = 0.175
- Content: Identity provider, AAD, authentication, security keywords (1.0) × 0.35 = 0.35
- Cross-reference: Referenced by `src/core/main.bicep` (0.85) × 0.10 = 0.085
...
- Path: `/src/core/`, `/src/inventory/` (0.70) × 0.25 = 0.175
- Content: RBAC, roleAssignments, principalId, roleDefinitionId keywords (1.0) × 0.35 = 0.35
- Cross-reference: Multiple files reference role definitions from `src/shared/constants.bicep` (0.75) × 0.10 = 0.075
...
- **Monitoring**: Self-monitoring loop — workspace sends its own diagnostic data to itself and to Storage Account (`src/shared/monitoring/operational/main.bicep:210-250`)
...
- Filename: `main.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/shared/monitoring/operational/` (0.60) × 0.25 = 0.15
- Content: `Microsoft.OperationalInsights/workspaces`, PerGB2018, monitoring, KQL keywords (1.0) × 0.35 = 0.35
- Cross-reference: Referenced by `insights/main.bicep`, `src/core/apim.bicep` as `logAnalyticsWorkspaceId` (0.90) × 0.10 = 0.09
...
- **Encryption**: Instrumentation key output marked `@secure()` in outputs — not exposed in plain text (`infra/main.bicep:130-133`)
...
- Filename: `main.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/shared/monitoring/insights/` (0.60) × 0.25 = 0.15
- Content: `Microsoft.Insights/components`, APM, telemetry, instrumentation keywords (1.0) × 0.35 = 0.35
- Cross-reference: Referenced by `infra/main.bicep` outputs, `src/core/apim.bicep` logger (0.90) × 0.10 = 0.09
...
- Filename: `apim.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/core/` (0.70) × 0.25 = 0.175
- Content: `SystemAssigned`, `identityType`, `principalId`, managed identity keywords (1.0) × 0.35 = 0.35
- Cross-reference: `common-types.bicep`, `constants.bicep` both reference identity patterns (0.95) × 0.10 = 0.095
...
- **Identity Config**: Conditional — only applied if `identity.type != 'None'` (`src/inventory/main.bicep:108-128`)
- **Last Rotated**: Azure-managed
- **EOL/EOS**: Platform feature; no EOL
...
- Filename: `main.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/inventory/` (0.50) × 0.25 = 0.125
- Content: `SystemAssigned`, identity configuration, principalId, role assignment keywords (1.0) × 0.35 = 0.35
- Cross-reference: `common-types.bicep`, `infra/main.bicep` reference (0.75) × 0.10 = 0.125
...
- Filename: `main.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/shared/monitoring/operational/` (0.60) × 0.25 = 0.15
- Content: `SystemAssigned`, identity configuration, LogAnalytics keywords (1.0) × 0.35 = 0.35
- Cross-reference: Referenced by `src/shared/monitoring/main.bicep` (0.80) × 0.10 = 0.08
...
- Filename: `workspaces.bicep` matches `*.bicep` (1.0) × 0.30 = 0.30
- Path: `/src/core/` (0.70) × 0.25 = 0.175
- Content: `Microsoft.ApiManagement/service/workspaces`, workspace isolation, API management keywords (1.0) × 0.35 = 0.35
- Cross-reference: Referenced by `src/core/main.bicep`, workspace name sourced from `infra/settings.yaml` (0.80) × 0.10 = 0.08
...
flowchart TD
    ...
    InfraMain("📄 infra/main.bicep<br/>Subscription Orchestrator"):::core
    SharedMod("📦 src/shared/main.bicep<br/>Monitoring Orchestrator"):::success
    OperationalMod("📦 operational/main.bicep<br/>Log Analytics + Storage"):::success
    InsightsMod("📦 insights/main.bicep<br/>Application Insights"):::success
    CoreMod("📦 src/core/main.bicep<br/>APIM Platform"):::core
    APIMBicep("📦 src/core/apim.bicep<br/>API Management Resource"):::core
    WorkspacesBicep("📦 src/core/workspaces.bicep<br/>APIM Workspaces"):::core
    DevPortalBicep("📦 src/core/developer-portal.bicep<br/>Portal Configuration"):::core
    InventoryMod("📦 src/inventory/main.bicep<br/>API Center Inventory"):::data

    subgraph AzureResources["☁️ Deployed Azure Resources"]
        LAWRes("📈 Log Analytics<br/>Workspace"):::success
        StorageRes("💾 Storage<br/>Account"):::neutral
        AppInsRes("🔍 Application<br/>Insights"):::success
        APIMRes("🔌 API Management<br/>Premium"):::core
        APIMWsRes("🗂️ APIM<br/>Workspace"):::core
        APICenterRes("📋 API<br/>Center"):::data
    end
