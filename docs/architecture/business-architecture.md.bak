...
**Confidence Range**: 0.70 – 0.85 (MEDIUM)
**Framework**: TOGAF 10 Business Architecture
...
| ------------------------- | ------ | -------------- | --------------------- |
| Business Strategy         | 1      | 0.72           | 3 – Defined           |
| Business Capabilities     | 10     | 0.75           | 3–4 – Defined/Managed |
| Value Streams             | 1      | 0.71           | 3 – Defined           |
| Business Processes        | 5      | 0.73           | 3 – Defined           |
| Business Services         | 5      | 0.78           | 3–4 – Defined/Managed |
| Business Functions        | 6      | 0.74           | 3–4 – Defined/Managed |
| Business Roles & Actors   | 6      | 0.74           | 3 – Defined           |
| Business Rules            | 8      | 0.81           | 4 – Managed           |
| Business Events           | 6      | 0.72           | 3 – Defined           |
| Business Objects/Entities | 8      | 0.76           | 3–4 – Defined/Managed |
| KPIs & Metrics            | 6      | 0.76           | 3–4 – Defined/Managed |
| **Total**                 | **62** | **0.74**       | **3.3 – Defined**     |
...
All eleven component type subsections are enumerated below. Each table uses the canonical five-column format: Name, Description, Source File, Confidence, and Maturity. Subsections with zero detected components are explicitly marked as "Not detected" per constraint N-5.

### 2.1 Business Strategy (1)

| Name                             | Description                                                                                                                                                    | Source File      | Confidence | Maturity    |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- | ---------- | ----------- |
| Enterprise API Platform Strategy | Strategic initiative to deliver a production-ready, governance-compliant Azure API Management landing zone in minutes rather than weeks through IaC automation | `README.md:1-20` | 0.72       | 3 – Defined |

### 2.2 Business Capabilities (10)

| Name                          | Description                                                                                           | Source File                               | Confidence | Maturity    |
| ----------------------------- | ----------------------------------------------------------------------------------------------------- | ----------------------------------------- | ---------- | ----------- |
| One-Command Deployment        | Full APIM landing zone provisioned with `azd up` — no manual Azure Portal steps required              | `README.md:200`, `azure.yaml:1`           | 0.75       | 4 – Managed |
| Configurable APIM SKUs        | Supports Developer, Basic, Standard, Premium, and Consumption tiers via YAML setting                  | `README.md:200`, `src/core/apim.bicep:75` | 0.75       | 3 – Defined |
| Managed Identity & Security   | System-assigned and user-assigned managed identity for all services; eliminates credential management | `src/shared/common-types.bicep:30`        | 0.76       | 4 – Managed |
| Integrated Observability      | Log Analytics + Application Insights + Storage, all pre-wired for diagnostics                         | `src/shared/monitoring/`                  | 0.77       | 4 – Managed |
| Developer Self-Service Portal | Azure AD-backed developer portal with CORS policies and MSAL 2.0 authentication                       | `src/core/developer-portal.bicep:1`       | 0.74       | 3 – Defined |
| Team Workspace Isolation      | APIM Workspaces for independent API lifecycle management per team (Premium SKU)                       | `src/core/workspaces.bicep:1`             | 0.74       | 3 – Defined |
| API Governance & Inventory    | Azure API Center with APIM auto-sync and RBAC role assignments                                        | `src/inventory/main.bicep:1`              | 0.78       | 4 – Managed |
| VNet Integration Ready        | External/Internal/None VNet modes configurable per deployment without Bicep modification              | `src/core/apim.bicep:96`                  | 0.75       | 3 – Defined |
| Governance Tagging            | Mandatory CostCenter, BusinessUnit, Owner, and RegulatoryCompliance tags on all resources             | `infra/settings.yaml:20`                  | 0.80       | 4 – Managed |
| Lifecycle Automation          | Pre-provision script automatically purges soft-deleted APIM instances to enable clean redeployment    | `infra/azd-hooks/pre-provision.sh:1`      | 0.73       | 3 – Defined |

### 2.3 Value Streams (1)

| Name                      | Description                                                                                                                                                      | Source File                          | Confidence | Maturity    |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ---------- | ----------- |
| API Platform Value Stream | End-to-end value delivery: repository clone → YAML configuration → `azd up` provisioning → operational governed API platform → team onboarding → API consumption | `README.md:30-60`, `azure.yaml:1-50` | 0.71       | 3 – Defined |

### 2.4 Business Processes (5)

| Name                              | Description                                                                                          | Source File                              | Confidence | Maturity    |
| --------------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------- | ---------- | ----------- |
| Environment Provisioning Process  | Clone repository → `az login` + `azd auth login` → `azd up` → verify environment outputs             | `README.md:30-60`                        | 0.73       | 3 – Defined |
| Soft-Delete Recovery Process      | List soft-deleted APIM instances → iterate and purge each → resume Bicep deployment                  | `infra/azd-hooks/pre-provision.sh:1-100` | 0.74       | 4 – Managed |
| Platform Configuration Process    | Edit `settings.yaml` fields → run `azd provision` → zero-code customization applied                  | `README.md:270-330`                      | 0.73       | 3 – Defined |
| Team Workspace Onboarding Process | Add workspace entries in `settings.yaml` → `azd provision` → workspace provisioned for team          | `README.md:350-380`                      | 0.72       | 3 – Defined |
| Contribution & Governance Process | Fork → branch → implement → validate with `az deployment sub what-if` → `az bicep build` → submit PR | `README.md:380-400`                      | 0.71       | 3 – Defined |

### 2.5 Business Services (5)

| Name                                  | Description                                                                                                                        | Source File                         | Confidence | Maturity    |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ---------- | ----------- |
| API Platform Delivery Service         | Automated subscription-scoped Bicep orchestration that creates resource groups and deploys all platform layers in dependency order | `infra/main.bicep:1`                | 0.78       | 4 – Managed |
| Monitoring & Observability Service    | Foundational diagnostics pipeline: Log Analytics + Application Insights + Storage, pre-wired to all APIM diagnostic settings       | `src/shared/main.bicep:1`           | 0.77       | 4 – Managed |
| API Governance Service                | Centralized API lifecycle governance via Azure API Center: workspace, API source linked to APIM, and RBAC role assignments         | `src/inventory/main.bicep:1`        | 0.78       | 4 – Managed |
| Developer Self-Service Portal Service | Azure AD-integrated developer portal: CORS global policy, AAD identity provider configuration, and sign-in/sign-up portal settings | `src/core/developer-portal.bicep:1` | 0.76       | 3 – Defined |
| Workspace Management Service          | APIM workspace provisioner: creates named, isolated workspaces as child resources of the APIM service for team collaboration       | `src/core/workspaces.bicep:1`       | 0.75       | 3 – Defined |

### 2.6 Business Functions (6)

| Name                        | Description                                                                                                              | Source File                          | Confidence | Maturity    |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------ | ---------- | ----------- |
| Infrastructure Provisioning | Subscription-scoped resource group creation and Bicep module deployment via `targetScope = 'subscription'`               | `infra/main.bicep:100`               | 0.70       | 3 – Defined |
| Configuration Management    | YAML-to-Bicep parameter injection via `loadYamlContent()` enabling zero-code environment customization                   | `infra/settings.yaml:1`              | 0.75       | 4 – Managed |
| API Lifecycle Management    | Publish, discover, sync, and govern APIs through integration of APIM and API Center                                      | `src/inventory/main.bicep:1`         | 0.76       | 3 – Defined |
| Monitoring & Diagnostics    | Centralized diagnostic stream connecting all services to Log Analytics and Application Insights                          | `src/shared/monitoring/main.bicep:1` | 0.72       | 4 – Managed |
| Security & Access Control   | Managed identity provisioning and RBAC role assignments (Reader, API Center Data Reader, Compliance Manager)             | `src/core/apim.bicep:60`             | 0.76       | 4 – Managed |
| Governance & Compliance     | Resource tag enforcement at group and resource levels; GDPR, CostCenter, ServiceClass, and RegulatoryCompliance metadata | `infra/settings.yaml:24`             | 0.80       | 4 – Managed |

### 2.7 Business Roles & Actors (6)

| Name                           | Description                                                                                        | Source File                          | Confidence | Maturity       |
| ------------------------------ | -------------------------------------------------------------------------------------------------- | ------------------------------------ | ---------- | -------------- |
| Platform Engineer              | Executes `azd up`, manages environment lifecycle; primary operator of the accelerator              | `README.md:12`, `azure.yaml:1`       | 0.74       | 3 – Defined    |
| Cloud Architect                | Designs API platform topology, selects SKU, configures VNet integration and governance standards   | `README.md:12`                       | 0.73       | 3 – Defined    |
| API Developer                  | Consumes developer portal for API documentation, testing, and self-service key registration        | `src/core/developer-portal.bicep:40` | 0.74       | 3 – Defined    |
| API Consumer                   | Invokes APIs through the APIM gateway; registered and authenticated via developer portal           | `src/core/developer-portal.bicep:40` | 0.72       | 2 – Repeatable |
| Operations Engineer            | Monitors platform health via Application Insights and Log Analytics; manages operational incidents | `src/shared/monitoring/main.bicep:1` | 0.74       | 3 – Defined    |
| Organization Owner / Publisher | Represents organizational identity; identified via `publisherName: "Contoso"` and `publisherEmail` | `infra/settings.yaml:40`             | 0.75       | 3 – Defined    |

### 2.8 Business Rules (8)

| Name                           | Description                                                                                                                        | Source File                                                  | Confidence | Maturity    |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ | ---------- | ----------- |
| BR-1 Environment Naming Rule   | Deployment environments MUST be one of: `dev`, `test`, `staging`, `prod`, `uat` — no custom values accepted                        | `infra/main.bicep:70`                                        | 0.80       | 4 – Managed |
| BR-2 SKU Workspace Rule        | Workspaces and VNet integration REQUIRE Premium SKU — unavailable on Basic, Standard, or Consumption tiers                         | `README.md:290`, `src/core/apim.bicep:75`                    | 0.82       | 4 – Managed |
| BR-3 Managed Identity Rule     | All services default to `SystemAssigned` managed identity; secrets-based authentication is not supported in standard configuration | `infra/settings.yaml:14`, `src/shared/common-types.bicep:30` | 0.80       | 4 – Managed |
| BR-4 Mandatory Tagging Rule    | All resources MUST carry: `CostCenter`, `BusinessUnit`, `Owner`, `ApplicationName`, and `RegulatoryCompliance` tags                | `infra/settings.yaml:20-34`                                  | 0.85       | 4 – Managed |
| BR-5 GDPR Compliance Rule      | The `RegulatoryCompliance` tag MUST be set to `"GDPR"` in all deployments                                                          | `infra/settings.yaml:27`                                     | 0.85       | 4 – Managed |
| BR-6 Soft-Delete Purge Rule    | Soft-deleted APIM instances MUST be purged before re-provisioning to the same region                                               | `infra/azd-hooks/pre-provision.sh:1`                         | 0.78       | 3 – Defined |
| BR-7 Service Class Rule        | Default service class is `"Critical"` — operators must explicitly override for non-critical workloads                              | `infra/settings.yaml:26`                                     | 0.80       | 4 – Managed |
| BR-8 Workspace Uniqueness Rule | Workspace names MUST be unique within the parent APIM service instance                                                             | `src/core/workspaces.bicep:1`                                | 0.76       | 3 – Defined |

### 2.9 Business Events (6)

| Name                       | Description                                                                                             | Source File                                           | Confidence | Maturity       |
| -------------------------- | ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ---------- | -------------- |
| Deployment Initiated       | `azd up` command triggers the full infrastructure provisioning workflow                                 | `azure.yaml:30`, `README.md:40`                       | 0.72       | 3 – Defined    |
| Pre-Provision Hook Fired   | `azd` preprovision lifecycle event triggers `pre-provision.sh` soft-delete cleanup                      | `azure.yaml:30`, `infra/azd-hooks/pre-provision.sh:1` | 0.74       | 3 – Defined    |
| Shared Monitoring Deployed | Monitoring foundation layer provisioned; Log Analytics and App Insights available for diagnostic wiring | `infra/main.bicep:110`                                | 0.73       | 3 – Defined    |
| APIM Platform Deployed     | Core API Management service ready; developer portal and workspace creation triggered                    | `infra/main.bicep:140`                                | 0.73       | 3 – Defined    |
| API Inventory Configured   | API Center linked to APIM; API source synchronized and RBAC roles assigned                              | `infra/main.bicep:165`                                | 0.72       | 3 – Defined    |
| Environment Decommissioned | `azd down` executed; all provisioned resources and resource group removed                               | `README.md:345`                                       | 0.70       | 2 – Repeatable |

### 2.10 Business Objects/Entities (8)

| Name                     | Description                                                                                                                      | Source File                                    | Confidence | Maturity    |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | ---------- | ----------- |
| Deployment Environment   | Named environment context (`dev`/`test`/`staging`/`prod`/`uat`) with associated region and resource group                        | `infra/main.bicep:70`, `infra/settings.yaml:1` | 0.75       | 4 – Managed |
| API Management Instance  | Core API gateway entity with `name`, `publisherEmail`, `publisherName`, `sku.name`, `sku.capacity`, `identity.type`              | `src/shared/common-types.bicep:74`             | 0.78       | 4 – Managed |
| API Workspace            | Logical isolation unit for team-based API development within a shared APIM instance                                              | `src/shared/common-types.bicep:74`             | 0.75       | 3 – Defined |
| API Inventory Entry      | Catalogued API artifact registered in Azure API Center for governance and discovery                                              | `src/inventory/main.bicep:60`                  | 0.76       | 3 – Defined |
| Governance Tag Set       | Mandatory metadata bundle: `CostCenter`, `BusinessUnit`, `Owner`, `RegulatoryCompliance`, `ServiceClass`, `BudgetCode`, and more | `infra/settings.yaml:20-34`                    | 0.80       | 4 – Managed |
| Publisher Identity       | Organization name (`publisherName: "Contoso"`) and contact email representing the API platform owner                             | `infra/settings.yaml:40-43`                    | 0.75       | 3 – Defined |
| Monitoring Configuration | Combined settings for `logAnalytics`, `applicationInsights`, and storage archival                                                | `src/shared/common-types.bicep:55`             | 0.75       | 4 – Managed |
| Solution Configuration   | Root YAML document (`settings.yaml`) parameterizing `solutionName`, `shared`, `core.apiManagement`, and `inventory.apiCenter`    | `infra/settings.yaml:1`                        | 0.76       | 4 – Managed |

### 2.11 KPIs & Metrics (6)

| Name                      | Description                                                                          | Source File                               | Confidence | Maturity    |
| ------------------------- | ------------------------------------------------------------------------------------ | ----------------------------------------- | ---------- | ----------- |
| Provisioning Speed        | Target: complete APIM landing zone operational in < 15 minutes from repository clone | `README.md:30`                            | 0.76       | 3 – Defined |
| Deployment Repeatability  | Zero manual Azure Portal steps required per deployment cycle                         | `README.md:33`                            | 0.78       | 4 – Managed |
| Compliance Tag Coverage   | 100% of deployed resources carry all mandatory governance tags                       | `infra/settings.yaml:20-34`               | 0.80       | 4 – Managed |
| Service Tier Compliance   | Premium SKU is the default; Developer SKU is explicitly flagged as non-SLA           | `README.md:286`, `infra/settings.yaml:46` | 0.76       | 3 – Defined |
| Soft-Delete Recovery Rate | 100% automated purge rate on re-provisioning — no manual intervention required       | `infra/azd-hooks/pre-provision.sh:1`      | 0.76       | 3 – Defined |
| API Center Sync Rate      | 100% of APIM-registered APIs discoverable in Azure API Center post-deployment        | `src/inventory/main.bicep:80`             | 0.74       | 3 – Defined |
...
| #      | Name                          | Type                 | Description                                                                                                                                                                        | Source File                               | Confidence | Key Relationships                                                                                               |
| ------ | ----------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------- |
| CAP-1  | One-Command Deployment        | Platform Delivery    | Full APIM landing zone provisioned with a single `azd up` command — no manual Azure Portal steps required post-provisioning                                                        | `README.md:200`, `azure.yaml:1`           | 0.75 (MED) | Enables: CAP-5, CAP-7; Realized by: API Platform Delivery Service; Source of: KPI-1, KPI-2                      |
| CAP-2  | Configurable APIM SKUs        | Platform Delivery    | Supports Developer, Basic, BasicV2, Standard, StandardV2, Premium, and Consumption SKU tiers via a single `settings.yaml` field change                                             | `README.md:200`, `src/core/apim.bicep:75` | 0.75 (MED) | Constrains: CAP-6 (Workspaces require Premium); Configured via: O-8 Solution Configuration                      |
| CAP-3  | Managed Identity & Security   | Security             | System-assigned and user-assigned managed identity for all services; eliminates credential management risk                                                                         | `src/shared/common-types.bicep:30`        | 0.76 (MED) | Secures: APIM Instance, API Center, Log Analytics; Enforces: BR-3; Enables: Security & Access Control function  |
| CAP-4  | Integrated Observability      | Operations           | Log Analytics workspace + Application Insights + Storage Account pre-wired with full APIM diagnostic settings                                                                      | `src/shared/monitoring/`                  | 0.77 (MED) | Provides diagnostics to: API Platform Delivery Service; Serves: Operations Engineer role; Enforces: Principle 4 |
| CAP-5  | Developer Self-Service Portal | Developer Experience | Azure AD-backed developer portal with CORS global policy, MSAL 2.0 library, and sign-in/sign-up portal flows                                                                       | `src/core/developer-portal.bicep:1`       | 0.74 (MED) | Serves: API Developer, API Consumer roles; Requires: Azure AD App Registration (external dependency)            |
| CAP-6  | Team Workspace Isolation      | Developer Experience | APIM Workspaces for independent API lifecycle management per team within a shared Premium SKU APIM instance                                                                        | `src/core/workspaces.bicep:1`             | 0.74 (MED) | Requires: Premium SKU (BR-2); Realized by: Workspace Management Service; Creates: API Workspace Object          |
| CAP-7  | API Governance & Inventory    | Governance           | Azure API Center with APIM auto-sync, default workspace, and RBAC role assignments (API Center Data Reader + Compliance Manager)                                                   | `src/inventory/main.bicep:1`              | 0.78 (MED) | Depends on: APIM Platform (Technology); Realized by: API Governance Service; Provides: KPI-6                    |
| CAP-8  | VNet Integration Ready        | Networking           | External, Internal, and None VNet modes fully configurable without any Bicep source modification                                                                                   | `src/core/apim.bicep:96`                  | 0.75 (MED) | Requires: Premium SKU for External/Internal modes (BR-2); Configured via: O-8 Solution Configuration            |
| CAP-9  | Governance Tagging            | Governance           | Mandatory CostCenter, BusinessUnit, Owner, ApplicationName, ProjectName, ServiceClass, RegulatoryCompliance, SupportContact, ChargebackModel, and BudgetCode tags on all resources | `infra/settings.yaml:20`                  | 0.80 (MED) | Enforces: BR-4, BR-5, BR-7; Realized by: Governance & Compliance function; Provides: KPI-3                      |
| CAP-10 | Lifecycle Automation          | Operations           | Pre-provision hook automatically purges soft-deleted APIM instances before reprovisioning to the same region                                                                       | `infra/azd-hooks/pre-provision.sh:1`      | 0.73 (MED) | Implements: Soft-Delete Recovery Process (P-2); Enforces: BR-6; Provides: KPI-5                                 |

### 5.3 Value Streams

| #    | Name                      | Description                                                                                                                                                                                                                             | Source File                          | Confidence | Key Relationships                                                                    |
| ---- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ---------- | ------------------------------------------------------------------------------------ |
| VS-1 | API Platform Value Stream | End-to-end value delivery: business requirement identified → YAML parameterization → `azd up` provisionng → operational governed API platform → team workspace onboarding → API publication and consumption → measurable business value | `README.md:30-60`, `azure.yaml:1-50` | 0.71 (MED) | Consists of: P-1 through P-4; Delivers: BS-1 through BS-5; Measured by: KPI-1, KPI-2 |
...
| #   | Name                      | Description                                                                                                                                                                                                                       | Source File                              | Confidence | Key Relationships                                                                                            |
| --- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------ |
| P-1 | Environment Provisioning  | Clone repo → `az login` + `azd auth login` → configure `settings.yaml` → `azd up` → verify environment outputs (`azd env get-values`)                                                                                             | `README.md:30-60`                        | 0.73 (MED) | Triggers: E-1, E-2; Uses: BS-1 API Platform Delivery Service; Creates: O-1 Deployment Environment            |
| P-2 | Soft-Delete Recovery      | `az apim deletedservice list` → iterate results → `az apim deletedservice purge` for each instance in target location → resume Bicep deployment                                                                                   | `infra/azd-hooks/pre-provision.sh:1-100` | 0.74 (MED) | Triggered by: E-2 Pre-Provision Hook Fired; Prevents: APIM naming conflicts on redeploy; Enforces: BR-6      |
| P-3 | Platform Configuration    | Identify configuration need → edit `infra/settings.yaml` fields → run `azd provision` → changes propagate to all modules via `loadYamlContent()`                                                                                  | `README.md:270-330`                      | 0.73 (MED) | Modifies: O-8 Solution Configuration; Invokes: BS-1 API Platform Delivery Service; No Bicep changes required |
| P-4 | Team Workspace Onboarding | Identify team isolation need → add workspace entry under `core.apiManagement.workspaces` in `settings.yaml` → run `azd provision` → workspace provisioned as child APIM resource                                                  | `README.md:350-380`                      | 0.72 (MED) | Creates: O-3 API Workspace Object; Uses: BS-5 Workspace Management Service; Requires: Premium SKU (BR-2)     |
| P-5 | Contribution & Governance | Fork repository on GitHub → create feature branch → implement changes following `src/` module structure → validate with `az deployment sub what-if` → build with `az bicep build` → submit PR with description and what-if output | `README.md:380-400`                      | 0.71 (MED) | Governed by: Architecture Principles 1–5; References: `src/shared/common-types.bicep` conventions            |

### 5.5 Business Services

| #    | Name                                  | Description                                                                                                                                                                                                                     | Source File                         | Confidence | Key Relationships                                                                                                                                                       |
| ---- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BS-1 | API Platform Delivery Service         | Subscription-scoped Bicep orchestration: creates resource group with `commonTags`, deploys Shared Monitoring → Core APIM → API Inventory in strict sequence, outputs resource IDs for downstream consumption                    | `infra/main.bicep:1`                | 0.78 (MED) | Consumed by: Platform Engineer; Delivers: O-1, O-2; Depends on: O-8 Solution Configuration; Orchestrates: BS-2, BS-3, BS-4, BS-5                                        |
| BS-2 | Monitoring & Observability Service    | Foundational diagnostics pipeline: provisions Log Analytics workspace (PerGB2018 SKU), Application Insights (Web type, LogAnalytics ingestion), and Storage Account (Standard_LRS); wires all three to APIM diagnostic settings | `src/shared/main.bicep:1`           | 0.77 (MED) | First deployed; outputs wired as inputs to BS-1; serves Operations Engineer role; provides O-7 Monitoring Configuration                                                 |
| BS-3 | API Governance Service                | Azure API Center service with `SystemAssigned` identity, default workspace, APIM API source integration (auto-discovery), and RBAC role assignments (API Center Data Reader + Compliance Manager)                               | `src/inventory/main.bicep:1`        | 0.78 (MED) | Depends on: BS-1 outputs (API Management resource ID + name); Governs: O-4 API Inventory Entries; Serves: Cloud Architect, Operations Engineer roles                    |
| BS-4 | Developer Self-Service Portal Service | APIM developer portal configuration: CORS global policy (all origins/methods/headers), Azure AD identity provider (MSAL-2, `login.windows.net`), sign-in and sign-up portal settings, terms of service                          | `src/core/developer-portal.bicep:1` | 0.76 (MED) | Depends on: existing APIM instance (Technology layer); External dep: Azure AD app registration; Serves: API Developer (R-3) and API Consumer (R-4) roles                |
| BS-5 | Workspace Management Service          | APIM workspace provisioner: creates named workspaces (`Microsoft.ApiManagement/service/workspaces`) with `displayName` and `description` as child resources of the APIM service instance                                        | `src/core/workspaces.bicep:1`       | 0.75 (MED) | Depends on: existing APIM instance (Technology layer); Creates: O-3 API Workspace Objects; Requires: Premium SKU (BR-2); Serves: Platform Engineer, API Developer roles |

### 5.6 Business Functions

| #    | Name                        | Description                                                                                                                                                                      | Source File                                             | Confidence | Key Relationships                                                                                                    |
| ---- | --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------- |
| BF-1 | Infrastructure Provisioning | Subscription-scoped resource group creation and module deployment orchestration using `targetScope = 'subscription'` with `commonTags` applied at group level                    | `infra/main.bicep:100`                                  | 0.70 (MED) | Realizes: BS-1; Governed by: BR-1 (Environment Naming); Principle: Principle 2 (Separation of Concerns)              |
| BF-2 | Configuration Management    | YAML-to-Bicep parameter injection via `loadYamlContent(settingsFile)` in `infra/main.bicep`; enables zero-code environment customization from a single YAML file                 | `infra/settings.yaml:1`, `infra/main.bicep:50`          | 0.75 (MED) | Enables: P-3 (Platform Configuration Process); Implements: Principle 1 (Declarative Configuration)                   |
| BF-3 | API Lifecycle Management    | Publish, discover, sync, and govern APIs through APIM gateway integration with API Center; APIM auto-registers as an API source in API Center default workspace                  | `src/inventory/main.bicep:1`                            | 0.76 (MED) | Realized by: BS-3 (API Governance Service); Creates: O-4 (API Inventory Entries); Provides: KPI-6 (sync rate)        |
| BF-4 | Monitoring & Diagnostics    | Configures diagnostic settings on all Azure resources (APIM, Application Insights, Storage) flowing to Log Analytics; 90-day retention for Application Insights                  | `src/shared/monitoring/main.bicep:1`                    | 0.72 (MED) | Realized by: BS-2 (Monitoring Service); Implements: Principle 4 (Observability First)                                |
| BF-5 | Security & Access Control   | Provisions managed identity for APIM, assigns Reader RBAC role (`acdd72a7`), assigns API Center Data Reader and Compliance Manager roles via `roleAssignments` resource          | `src/core/apim.bicep:60`, `src/inventory/main.bicep:80` | 0.76 (MED) | Implements: Principle 3 (Security by Default), BR-3 (Managed Identity Rule); Enables: BS-3 governance                |
| BF-6 | Governance & Compliance     | Enforces 10-field governance tag set on all resources via `commonTags` union; GDPR, CostCenter, ServiceClass, and BudgetCode metadata applied at resource group and module level | `infra/settings.yaml:24`, `infra/main.bicep:75`         | 0.80 (MED) | Enforces: BR-4, BR-5, BR-7; Implements: Principle 5 (Governance-Driven Management); Serves: R-6 (Organization Owner) |

### 5.7 Business Roles & Actors

| #   | Name                           | Type            | Responsibilities                                                                                                                                  | Source File                          | Confidence | Key Relationships                                                                                               |
| --- | ------------------------------ | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ---------- | --------------------------------------------------------------------------------------------------------------- |
| R-1 | Platform Engineer              | Internal Human  | Executes `azd up`; manages deployment environments (new, provision, down); configures `settings.yaml` for each target environment                 | `README.md:12`, `azure.yaml:1`       | 0.74 (MED) | Executes: P-1 (Provisioning Process); Uses: BS-1 (API Platform Delivery Service)                                |
| R-2 | Cloud Architect                | Internal Human  | Designs API platform topology; selects APIM SKU; configures VNet integration type; governs architecture standards; reviews Bicep module structure | `README.md:12`                       | 0.73 (MED) | Defines: Architecture Principles 1–5; Configures: O-8 (Solution Config); Owns: S-1 (Enterprise Strategy)        |
| R-3 | API Developer                  | Internal Human  | Consumes developer portal for API documentation, endpoint testing, OAuth2 key registration, and self-service subscription management              | `src/core/developer-portal.bicep:40` | 0.74 (MED) | Uses: BS-4 (Developer Portal Service), BS-5 (Workspace Management); Governed by: Azure AD identity              |
| R-4 | API Consumer                   | External Human  | Invokes APIs through the APIM gateway endpoint; discovers available APIs via API Center and developer portal catalog                              | `src/core/developer-portal.bicep:40` | 0.72 (MED) | Accesses: APIM Gateway (Technology layer); Discovers via: BS-3 (API Governance Service)                         |
| R-5 | Operations Engineer            | Internal Human  | Monitors API platform health, performance trends, and error rates via Application Insights and Log Analytics; responds to operational events      | `src/shared/monitoring/main.bicep:1` | 0.74 (MED) | Uses: BS-2 (Monitoring Service); Responds to: E-3 through E-6 (Business Events)                                 |
| R-6 | Organization Owner / Publisher | Internal/System | Represents organizational API platform ownership; identified by `publisherName: "Contoso"` and `publisherEmail` in APIM configuration             | `infra/settings.yaml:40`             | 0.75 (MED) | Owns: O-2 (API Management Instance), O-6 (Publisher Identity); Enforces: BR-4 (Mandatory Tagging via Owner tag) |

### 5.8 Business Rules

| #    | Rule Name                 | Rule Statement                                                                                                                                                                                                                           | Enforcement Mechanism                                                                                                               | Source File                                                  | Confidence |
| ---- | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ | ---------- |
| BR-1 | Environment Naming Rule   | Deployment environments MUST be one of: `dev`, `test`, `staging`, `prod`, `uat`. No custom environment names are accepted.                                                                                                               | `@allowed([...])` Bicep parameter constraint rejecting non-listed values at deploy time                                             | `infra/main.bicep:70`                                        | 0.80 (MED) |
| BR-2 | SKU Workspace Rule        | APIM Workspaces and VNet integration (External/Internal modes) REQUIRE Premium SKU. These features are architecturally unavailable on Basic, Standard, or Consumption tiers.                                                             | Documentation constraint (README.md:290); enforced by Azure API at resource creation time                                           | `README.md:290`, `src/core/apim.bicep:75`                    | 0.82 (MED) |
| BR-3 | Managed Identity Rule     | All Azure services default to `SystemAssigned` managed identity. Secrets-based authentication is not supported in the standard accelerator configuration.                                                                                | Type system in `common-types.bicep`; default values in `settings.yaml`                                                              | `infra/settings.yaml:14`, `src/shared/common-types.bicep:30` | 0.80 (MED) |
| BR-4 | Mandatory Tagging Rule    | All resources MUST carry a complete governance tag set including `CostCenter`, `BusinessUnit`, `Owner`, `ApplicationName`, `ProjectName`, `ServiceClass`, `RegulatoryCompliance`, `SupportContact`, `ChargebackModel`, and `BudgetCode`. | `commonTags` union applied at resource group level in `infra/main.bicep:75`; propagated to all modules via `tags` parameter         | `infra/settings.yaml:20-34`                                  | 0.85 (MED) |
| BR-5 | GDPR Compliance Rule      | The `RegulatoryCompliance` tag MUST be set to `"GDPR"` in all deployments. Operators are responsible for maintaining this setting when forking the accelerator.                                                                          | Default value `RegulatoryCompliance: "GDPR"` in `settings.yaml:27`; governance responsibility of Organization Owner                 | `infra/settings.yaml:27`                                     | 0.85 (MED) |
| BR-6 | Soft-Delete Purge Rule    | Soft-deleted APIM instances MUST be purged before reprovisioning to the same Azure region. Failure to do so causes naming conflicts and deployment failure.                                                                              | Automated enforcement via `infra/azd-hooks/pre-provision.sh` which runs `az apim deletedservice purge` before every `azd provision` | `infra/azd-hooks/pre-provision.sh:1`                         | 0.78 (MED) |
| BR-7 | Service Class Rule        | Default service class is `"Critical"`. Operators MUST explicitly downgrade to `"Standard"` or `"Experimental"` for non-critical workloads to avoid incorrect cost and SLA expectations.                                                  | Default value in `settings.yaml:26`; propagated via `commonTags`; operator responsibility to override                               | `infra/settings.yaml:26`                                     | 0.80 (MED) |
| BR-8 | Workspace Uniqueness Rule | Workspace names MUST be unique within the parent APIM service instance. Duplicate workspace names cause `workspaces.bicep` deployment failures.                                                                                          | Azure API resource uniqueness constraint at `Microsoft.ApiManagement/service/workspaces` resource type                              | `src/core/workspaces.bicep:1`                                | 0.76 (MED) |

### 5.9 Business Events

| #   | Name                       | Trigger Condition                                         | Business Effect                                                                                                                    | Source File                                           | Confidence |
| --- | -------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ---------- |
| E-1 | Deployment Initiated       | Platform engineer executes `azd up` command               | `azure.yaml` preprovision hook lifecycle begins; provisioning workflow enters first stage                                          | `azure.yaml:30`, `README.md:40`                       | 0.72 (MED) |
| E-2 | Pre-Provision Hook Fired   | `azd` preprovision lifecycle event fires                  | `pre-provision.sh` script enumerates and purges soft-deleted APIM instances in target location                                     | `azure.yaml:30`, `infra/azd-hooks/pre-provision.sh:1` | 0.74 (MED) |
| E-3 | Shared Monitoring Deployed | `deploy-shared-components` module deployment completes    | Log Analytics workspace, Application Insights, and Storage Account available; diagnostic wiring enabled for downstream services    | `infra/main.bicep:110`                                | 0.73 (MED) |
| E-4 | APIM Platform Deployed     | `deploy-core-platform` module deployment completes        | API Management service, developer portal, and workspaces operational; APIM resource ID output available for API Center integration | `infra/main.bicep:140`                                | 0.73 (MED) |
| E-5 | API Inventory Configured   | `deploy-inventory-components` module deployment completes | API Center default workspace created; APIM registered as API source; API discovery active; RBAC roles assigned                     | `infra/main.bicep:165`                                | 0.72 (MED) |
| E-6 | Environment Decommissioned | Platform engineer executes `azd down`                     | All provisioned Azure resources and resource group are permanently removed; API Platform Value Stream ends for this environment    | `README.md:345`                                       | 0.70 (MED) |

### 5.10 Business Objects/Entities

| #   | Name                     | Key Attributes                                                                                                                                                                                                                 | Source File                                                        | Confidence | Key Relationships                                                                                                |
| --- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------ | ---------- | ---------------------------------------------------------------------------------------------------------------- |
| O-1 | Deployment Environment   | `envName` (dev/test/staging/prod/uat), `location`, `rgName` (pattern: `{solutionName}-{env}-{location}-rg`)                                                                                                                    | `infra/main.bicep:70`, `infra/settings.yaml:1`                     | 0.75 (MED) | Container for all deployed objects; created by: P-1; parameterized via: O-8                                      |
| O-2 | API Management Instance  | `name`, `publisherEmail`, `publisherName`, `sku.name`, `sku.capacity`, `identity.type`, `virtualNetworkType`                                                                                                                   | `src/shared/common-types.bicep:74`                                 | 0.78 (MED) | Core Business Object; owned by: R-6 (Organization Owner); serves: R-3, R-4                                       |
| O-3 | API Workspace            | `name` (unique within APIM), `displayName`, `description: 'Workspace for API development and management'`                                                                                                                      | `src/shared/common-types.bicep:74`, `src/core/workspaces.bicep:30` | 0.75 (MED) | Child of: O-2 (API Management Instance); created by: P-4 (Workspace Onboarding); governed by: BR-8               |
| O-4 | API Inventory Entry      | API source linked to APIM, workspace `default`, API Center service name, API source resource                                                                                                                                   | `src/inventory/main.bicep:60`                                      | 0.76 (MED) | Managed by: BS-3 (API Governance Service); discoverable by: R-4 (API Consumer); measured by: KPI-6               |
| O-5 | Governance Tag Set       | `CostCenter: "CC-1234"`, `BusinessUnit: "IT"`, `Owner: "evilazaro@gmail.com"`, `ApplicationName`, `ProjectName`, `ServiceClass: "Critical"`, `RegulatoryCompliance: "GDPR"`, `SupportContact`, `ChargebackModel`, `BudgetCode` | `infra/settings.yaml:20-34`                                        | 0.80 (MED) | Applied to all Azure resources via `commonTags`; enforces: BR-4, BR-5, BR-7                                      |
| O-6 | Publisher Identity       | `publisherName: "Contoso"`, `publisherEmail: "evilazaro@gmail.com"`                                                                                                                                                            | `infra/settings.yaml:40-43`                                        | 0.75 (MED) | Identifies: R-6 (Organization Owner); registered on: O-2 (APIM Instance)                                         |
| O-7 | Monitoring Configuration | `logAnalytics.name`, `logAnalytics.identity.type`, `applicationInsights.name`, `logAnalyticsWorkspaceResourceId`, `tags.component: "monitoring"`                                                                               | `src/shared/common-types.bicep:55`                                 | 0.75 (MED) | Consumed by: BS-2 (Monitoring Service); references: Log Analytics workspace (Technology layer)                   |
| O-8 | Solution Configuration   | `solutionName: "apim-accelerator"`, `shared` (monitoring + tags), `core.apiManagement`, `inventory.apiCenter`                                                                                                                  | `infra/settings.yaml:1`                                            | 0.76 (MED) | Master configuration object; loaded by `infra/main.bicep` via `loadYamlContent()`; parameterizes all deployments |

### 5.11 KPIs & Metrics

| #     | Name                      | Target                                                                                 | Measurement Method                                                                            | Source File                               | Confidence | Key Relationships                                                                  |
| ----- | ------------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ----------------------------------------- | ---------- | ---------------------------------------------------------------------------------- |
| KPI-1 | Provisioning Speed        | < 15 minutes from repository clone to operational APIM landing zone                    | Wall-clock time from first `azd up` invocation to `Deployment completed` in output            | `README.md:30`                            | 0.76 (MED) | Measures: VS-1 (API Platform Value Stream) efficiency; depends on: BS-1 throughput |
| KPI-2 | Deployment Repeatability  | Zero manual Azure Portal steps per deployment cycle                                    | Checklist audit: all steps in quick start guide are CLI-based; no Portal actions documented   | `README.md:33`                            | 0.78 (MED) | Measures: CAP-1 (One-Command Deployment) completeness                              |
| KPI-3 | Compliance Tag Coverage   | 100% of deployed resources carry all 10 mandatory governance tags                      | `az tag list` after deployment; verify each resource has complete O-5 tag set                 | `infra/settings.yaml:20-34`               | 0.80 (MED) | Measures: CAP-9 (Governance Tagging) and BF-6 (Governance & Compliance function)   |
| KPI-4 | Service Tier Compliance   | Premium SKU selected for production environments; Developer SKU only in non-production | SKU name verified in `infra/settings.yaml:46` prior to prod deployment                        | `README.md:286`, `infra/settings.yaml:46` | 0.76 (MED) | Measures: CAP-2 (Configurable APIM SKUs) governance adherence                      |
| KPI-5 | Soft-Delete Recovery Rate | 100% automated purge success rate — no manual intervention required on redeploy        | Pre-provision hook exit code 0; verify `az apim deletedservice list` returns empty after hook | `infra/azd-hooks/pre-provision.sh:1`      | 0.76 (MED) | Measures: CAP-10 (Lifecycle Automation) and P-2 (Soft-Delete Recovery Process)     |
| KPI-6 | API Center Sync Rate      | 100% of APIM-registered APIs discoverable in Azure API Center post-deployment          | Count APIs in APIM vs. API Source in API Center; verify auto-discovery active                 | `src/inventory/main.bicep:80`             | 0.74 (MED) | Measures: CAP-7 (API Governance & Inventory) and BF-3 (API Lifecycle Management)   |
...
| Business Component                           | Depends On                                                     | Layer                 | Integration Type                                                       | Source File                          |
| -------------------------------------------- | -------------------------------------------------------------- | --------------------- | ---------------------------------------------------------------------- | ------------------------------------ |
| API Platform Delivery Service (BS-1)         | Azure Subscription                                             | Technology / External | ARM Resource Group deployment at subscription scope                    | `infra/main.bicep:100`               |
| Monitoring & Observability Service (BS-2)    | Log Analytics Workspace, Application Insights, Storage Account | Technology            | Resource deployment + diagnostic settings wiring                       | `src/shared/main.bicep:50`           |
| API Governance Service (BS-3)                | APIM service, Azure API Center                                 | Technology            | ARM resource + API Source integration with RBAC                        | `src/inventory/main.bicep:60`        |
| Developer Self-Service Portal Service (BS-4) | Azure Active Directory, APIM service                           | External / Technology | AAD identity provider config + CORS policy                             | `src/core/developer-portal.bicep:40` |
| Workspace Management Service (BS-5)          | APIM service                                                   | Technology            | Child resource creation (`Microsoft.ApiManagement/service/workspaces`) | `src/core/workspaces.bicep:30`       |
| Environment Provisioning Process (P-1)       | Azure CLI, azd CLI                                             | External Tools        | CLI command execution; azd lifecycle orchestration                     | `azure.yaml:30`                      |
| Soft-Delete Recovery Process (P-2)           | Azure CLI, APIM soft-delete API                                | External / Technology | `az apim deletedservice list` + purge API calls                        | `infra/azd-hooks/pre-provision.sh:1` |
| Governance Tagging (CAP-9, BF-6)             | Azure Resource Manager                                         | Technology            | Tag propagation via ARM resource `tags` property                       | `infra/main.bicep:75`                |
